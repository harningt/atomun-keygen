...
import us.eharning.atomun.keygen.*
 ...
 * @since 0.0.1
 */
internal class BIP0044KeyGeneratorBuilderSpi : KeyGeneratorBuilderSpi(StandardKeyGeneratorAlgorithm.BIP0044) {
    ...
    override fun validate(vararg parameters: BuilderParameter?) {
        ...
        for (parameter in parameters) {
            ...
            when (parameter) {
                is BIP0032Path -> {
                    BIP0044Path.checkValidPath(parameter)
                }
                is ByteArraySeedParameter,
                is SerializedSeedParameter -> {}
                else -> throw IllegalArgumentException("Unsupported parameter type: " + parameter)
